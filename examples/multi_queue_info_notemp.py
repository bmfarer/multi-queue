...
def get_train_loader(args, dataset, height, width, batch_size, workers,
    ...
    train_transformer = T.Compose([
        ...
        T.ToTensor(),
        normalizer,
        # T2.RandomErasing(probability=0.5, sl=0.02, sh=0.5, r1=0.4, mean=[0.485, 0.456, 0.406])
        T.RandomErasing(probability=0.5, mean=[0.485, 0.456, 0.406])
    ])
    # print('T2.RandomErasing(probability=0.5,sl=0.1,sh=0.5,r1=0.4, mean=[0.485, 0.456, 0.406])')
...
def main_worker(args):
    ...
    centroids = Queues(momentum=args.momentum, sample_num=args.sample_num)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--logs-dir', type=str, metavar='PATH',
                        default=osp.join(working_dir, 'logs/multi_queue_info_notemp'))
