...
def main_worker(args):
    ...
    # Trainer
    trainer = ClusterContrastTrainer(model,centroid_weight=args.cent_weight)

    # DBSCAN cluster
    # print('Clustering criterion: eps: {:.3f}'.format(eps))
    # cluster = DBSCAN(eps=eps, min_samples=4, metric='precomputed', n_jobs=-1)
    centroids = Queues(momentum=args.momentum, sample_num=args.sample_num, noise_weight=args.q_noise)
    ...
    # optimizer.add_param_group({"params": memory.temp_mlp.parameters(), "lr": args.lr*args.tlr, "weight_decay": args.weight_decay})
    # optimizer.add_param_group({"params": memory.temp, "lr": args.lr * args.tlr})
    # optimizer.add_param_group({"params": centroids.feature_queues, "lr": args.lr * 0.01})

    # optimizer_enc.add_param_group({"params": memory.temp, "lr": args.lr / 10})
    ...
    for epoch in range(last_epoch, args.epochs):
        ...
        # centroids = Centroids(pseudo_labels, queues, momentum=args.momentum, sample_num=args.sample_num)
        # cluster_features = generate_cluster_features(pseudo_labels, queues)
        centroids.init_queue(features, pseudo_labels)
        del cluster_loader, features

        # Create hybrid memory
        # memory = ClusterMemory(centroids, temp=args.temp,
        #                        momentum=args.momentum, use_hard=args.use_hard).cuda()
        memory.refresh(centroids)
        # memory.queues = F.normalize(cluster_features, dim=1).cuda()
        # memory.temp=torch.nn.Parameter(torch.tensor(args.temp))
        # optimizer_enc.add_param_group({"params": memory.temp, "lr": args.lr / 10})
    ...
    print('==> Test with the best encoder:')
    checkpoint = load_checkpoint(osp.join(args.logs_dir, 'model_best.pth.tar'))
    model.load_state_dict(checkpoint['state_dict'])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--eps', type=float, default=0.5,
                        help="min similarity distance for Infomap")
    ...
    parser.add_argument('--logs-dir', type=str, metavar='PATH',
                        default=osp.join(working_dir, 'logs/multi_info'))
    parser.add_argument('--pooling-type', type=str, default='gem')
    parser.add_argument('--use-hard', action="store_true")
    parser.add_argument('--no-cam', action="store_true")

    parser.add_argument('--load-last', type=bool, default=False,
                        help="whether loaded the last training best encoder")
    ...
    parser.add_argument('--gamma', default=0.1, type=float, help='the lr gamma')
    parser.add_argument('--cent_weight',type=float,default=0.1,help='centroid weight')
    parser.add_argument('--tlr',default=0.01,type=float,help='temp learning rate')
    parser.add_argument('--q-noise', default=0.15, type=float, help='noise weight of multi-queue')
    main()
